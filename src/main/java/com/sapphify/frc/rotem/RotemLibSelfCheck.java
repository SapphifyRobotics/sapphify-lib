 ...
 *
 * <pre>{@code
 * cd src/main/java && java com/sapphify/frc/rotem/RotemLibSelfCheck.java
 * }</pre>
 *
 ...
 * {@link RotemSimTransport} owning the clock; nothing in the library reads a wall clock.
 */
public class RotemLibSelfCheck {
  static int fail = 0;
  ...
  public static void main(String[] a) {
    var t = new RotemSimTransport();
    var imu = new CoreRotemAhrs(0, t);
    check("no frame -> DEVICE_NOT_PRESENT", imu.getYaw().status()==RotemStatusCode.DEVICE_NOT_PRESENT);
    int orient = RotemProtocol.arbitrationId(RotemProtocol.DeviceType.AHRS, RotemProtocol.Api.STATUS_ORIENTATION, 0);
    ...
    check("short frame -> MALFORMED_FRAME", imu.getYaw().status()==RotemStatusCode.MALFORMED_FRAME);
    int health=RotemProtocol.arbitrationId(RotemProtocol.DeviceType.AHRS,RotemProtocol.Api.STATUS_HEALTH,0);
    int flags=RotemProtocol.Flag.HIGH_VIBRATION|RotemProtocol.Flag.MAG_DISTURBED;
    ...
    check("identify(0) rejected", imu.identify(0)==RotemStatusCode.INVALID_PARAMETER);
    check("identify(5) ok", imu.identify(5).isOK());
    var cfg=new RotemAhrsConfiguration();
    ...
    check("out-of-range mount -> INVALID_PARAMETER", imu.getConfigurator().apply(cfg)==RotemStatusCode.INVALID_PARAMETER);
    cfg.mountPose.yawDegrees=90;
    check("valid config applies", imu.getConfigurator().apply(cfg).isOK());
    var t2=new RotemSimTransport(); var imu2=new CoreRotemAhrs(1,t2);
    t2.advanceTime(10.0);
    var good=new RotemAhrsConfiguration();
    RotemStatusCode last=RotemStatusCode.OK;
    for(int i=0;i<200;i++){ last=imu2.getConfigurator().apply(good); t2.advanceTime(0.02); }
    check("4s of loop-rate applies -> FREQUENT_CONFIG_CALLS", last==RotemStatusCode.FREQUENT_CONFIG_CALLS);
    check("null transport -> NO_TRANSPORT", new CoreRotemAhrs(2,null).zeroYaw()==RotemStatusCode.NO_TRANSPORT);
    boolean threw=false; try{ new CoreRotemAhrs(63,t);}catch(IllegalArgumentException e){threw=true;}
    check("device 63 reserved -> throws", threw);
    check("status codes carry remedies", RotemStatusCode.DEVICE_NOT_PRESENT.description().contains("60 ohms"));
    check("setUpdateFrequency reaches transport",
        imu.setUpdateFrequency(RotemProtocol.Api.STATUS_ORIENTATION,250).isOK()
